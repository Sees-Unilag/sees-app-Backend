import {
  ...
  export class FileValidationPipe implements PipeTransform {
    transform(files: Express.Multer.File[] | Express.Multer.File, _metadata: ArgumentMetadata) {
      if(!files) { throw new BadRequestException("Please upload a file!") }
      if (Array.isArray(files)){
        for(const file of files){
          this.validateFileSize(file);
          const allowedExtension = ['doc','jpg','jpeg', 'png','docx','ppt','pptx','pdf'];
          const errorMessage = 'Invalid File Format, We only support pdfs, images, documents and presentations';
          this.validateExtension(file, errorMessage, allowedExtension );
          }}
      return files;
    }
    protected validateExtension(file:Express.Multer.File, errorMessage:string, extensions:string[]){
